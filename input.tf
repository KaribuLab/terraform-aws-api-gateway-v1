...
# ============================================================================
# Integraciones Lambda (nueva interfaz declarativa)
# ============================================================================

variable "lambda_integrations" {
  description = <<-EOT
    Lista de integraciones Lambda para el API Gateway.
    Cada integración define un endpoint (path + método HTTP) conectado a una función Lambda.
    
    Campos:
    - path: Ruta del endpoint (ej: "/users", "/profile/{id}")
    - method: Método HTTP (GET, POST, PUT, DELETE, PATCH, OPTIONS)
    - lambda_invoke_arn: ARN de invocación de la función Lambda (requerido)
    - lambda_function_arn: ARN completo de la función Lambda para permisos (requerido)
    - authorization_type: Tipo de autorización (NONE, AWS_IAM, CUSTOM). Default: NONE
    - authorizer_key: Clave del authorizer definido en var.authorizers (opcional, solo si authorization_type = CUSTOM)
    - api_key_required: Si requiere API Key. Default: false
    - request_parameters: Parámetros de request (ej: {"method.request.path.id" = true})
    - enable_cors: Habilitar CORS (crea método OPTIONS automáticamente). Default: false
  ...
  validation {
    condition = alltrue([
      ...
      contains(["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS", "HEAD", "ANY"], integration.method)
    ])
    error_message = "El método HTTP debe ser uno de: GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD, ANY."
  }

  validation {
    condition = alltrue([
      for integration in var.lambda_integrations :
      contains(["NONE", "AWS_IAM", "CUSTOM"], integration.authorization_type)
    ])
    error_message = "authorization_type debe ser uno de: NONE, AWS_IAM, CUSTOM."
  }

  validation {
    condition = alltrue([
      for integration in var.lambda_integrations :
      integration.authorization_type != "CUSTOM" || integration.authorizer_key != null
    ])
...
variable "authorizers" {
  description = <<-EOT
    ...
    Campos:
    - lambda_arn: ARN de la función Lambda authorizer
    - type: Tipo de authorizer (TOKEN o REQUEST). Default: TOKEN
    - identity_source: Fuente de identidad. Default: "method.request.header.Authorization"
    - authorizer_result_ttl: TTL del resultado en segundos. Default: 300
    - identity_validation_expression: Expresión regex de validación (opcional)
  EOT
  type = map(object({
    lambda_arn                     = string
...
variable "deployment_description" {
  ...
  type        = string
  default     = "Deployment managed by Terraform"
}
